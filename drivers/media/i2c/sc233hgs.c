...
#define SC233HGS_LINK_FREQ_371M       371250000ULL
#define SC233HGS_LANES                2
#define SC233HGS_BITS_PER_SAMPLE      10
#define SC233HGS_PIXEL_RATE           (SC233HGS_LINK_FREQ_371M * 2 * \
...
#define SC233HGS_REG_CTRL_MODE        0x0100
...
/*
 * Recovered from the .data table at 0xffffffc009221978. The binary iterates
 * over 4-byte entries as { u16 reg; u8 val; u8 pad } until reg == 0xffff.
 * This is the single built-in 1928x1080 10-bit mode present in the image.
 */
static const struct sc233hgs_regval sc233hgs_1928x1080_regs[] = {
	{0x0100, 0x00}, {0x36e9, 0x80}, {0x37f9, 0x80}, {0x300c, 0x24},
	{0x3018, 0x3a}, {0x3019, 0x0c}, {0x301f, 0x7a}, {0x3044, 0x10},
	{0x3062, 0x00}, {0x3200, 0x00}, {0x3201, 0x00}, {0x3202, 0x00},
	{0x3203, 0x3c}, {0x3204, 0x07}, {0x3205, 0x8f}, {0x3206, 0x04},
	{0x3207, 0x83}, {0x3208, 0x07}, {0x3209, 0x88}, {0x320a, 0x04},
	{0x320b, 0x38}, {0x320c, 0x09}, {0x320d, 0x60}, {0x320e, 0x04},
	{0x320f, 0x65}, {0x3210, 0x00}, {0x3211, 0x04}, {0x3212, 0x00},
	{0x3213, 0x08}, {0x321f, 0x0b}, {0x322f, 0x00}, {0x3222, 0x01},
	{0x3282, 0x00}, {0x3231, 0x00}, {0x3250, 0x03}, {0x32e2, 0x00},
	...
	{0x36c6, 0x08}, {0x36c7, 0x38}, {0x36ea, 0x0b}, {0x36eb, 0x0c},
	{0x36ec, 0x53}, {0x36ed, 0x24}, {0x3718, 0x04}, {0x3719, 0x06},
	...
	{0x37d4, 0x08}, {0x37d5, 0x38}, {0x37fa, 0x03}, {0x37fb, 0xe5},
	...
	{0x3e00, 0x00}, {0x3e01, 0x40}, {0x3e02, 0x00}, {0x3e03, 0x0b},
	...
	{0x4373, 0x38}, {0x4412, 0x17}, {0x4424, 0x01}, {0x450d, 0x10},
	{0x4837, 0x2b}, {0x4b0a, 0x92}, {0x5000, 0x38}, {0x5002, 0x00},
	{0x502e, 0x21}, {0x5034, 0x01},
	{REG_NULL, 0x00},
};

static const s64 sc233hgs_link_freq_menu_items[] = {
	SC233HGS_LINK_FREQ_371M,
};
...
static const struct sc233hgs_mode sc233hgs_supported_modes[] = {
	{
		.width = 1928,
		.height = 1080,
		.max_fps = { .numerator = 10000, .denominator = 300000 },
		.hts_def = 0x0960,
		.vts_def = 0x0465,
		.exp_def = 0x0040,
		.bus_fmt = MEDIA_BUS_FMT_SBGGR10_1X10,
		.reg_list = sc233hgs_1928x1080_regs,
		.hdr_mode = NO_HDR,
		.vc = { 0, 1, 2, 3 },
		.link_freq_idx = 0,
	},
...
static int sc233hgs_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		if (!ret && sc->cur_mode->hdr_mode == NO_HDR)
			ret = sc233hgs_write_reg(sc->client, SC233HGS_REG_CTRL_MODE,
...
static long sc233hgs_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_QUICK_STREAM:
		return sc233hgs_write_reg(sc->client, SC233HGS_REG_CTRL_MODE,
					  SC233HGS_REG_VALUE_08BIT,
					  *(u32 *)arg ? SC233HGS_MODE_STREAMING :
							SC233HGS_MODE_SW_STANDBY);
	default:
		return -ENOIOCTLCMD;
	}
